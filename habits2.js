    ...
    userHabits.length === 0
      ...
      : userHabits
        ...
        <li class="shadow border rounded bg-light mx-auto m-4 p-3">
          ...
          <button class="btn btn-light shadow border btn-decrement${disabled}" data-index="${i}" data-streak="${
              habit.streak
            }"><i class="fa-solid fa-minus"></i></button>
          <span>${habit.streak}</span>
          <button class="btn btn-light shadow border btn-increment" data-index="${i}"> <i class="fa-solid fa-plus"></i></button>
          <button class="btn btn-light shadow border btn-reset ${disabled}" data-index="${i}" data-streak="${
              habit.streak
...
function deleteHabit(e) {
  ...
  let habits = JSON.parse(localStorage.getItem("habits")) || [];
  let userHabits = habits.filter((habit) => habit.userId === loggedInUserId);

  if (index >= 0 && index < userHabits.length) {
    userHabits.splice(index, 1);
    listHabits(userHabits, habitsList);
    // Update habits array in localStorage with the modified userHabits
    habits = habits.filter((habit) => habit.userId !== loggedInUserId);
...
///////HÄR ÄR NY KOD
// Update habits array in localStorage with the modified userHabits
// function updateHabitsLocalStorage() {
//   habits = habits.filter((habit) => habit.userId !== loggedInUserId);
//   habits.push(...userHabits);
//   localStorage.setItem("habits", JSON.stringify(habits));
// }
