...
function deleteHabit(e) {
  ...
  // Find the index of the habit with the matching text (Approach 2)
  const index = habits.findIndex(habit => habit.text === text);

  if (index !== -1) {
    // Efficiently filter and update habits for loggedInUserId (Approach 1)
    const userHabits = habits.filter(habit => habit.userId !== loggedInUserId);
    userHabits.push(...habits.filter(habit => habit.text === text && habit.userId === loggedInUserId));

    // Update the main habits array (Approach 2)
    habits.splice(index, 1);

    listHabits(userHabits, habitsList); // Update habit list with filtered userHabits (Approach 1)

    localStorage.setItem("habits", JSON.stringify(habits)); // Save habits to localStorage (Approach 2)
  }
}
