...
todoForm.addEventListener("submit", function (event) {
  ...
  let categoryIndexText =
    todoForm.modalCategory.options[categoryIndex].textContent;
  let category = categoryIndexText;

  let estTimeIndex = todoForm.modalEstTime.selectedIndex;
  let estTimeIndexText =
    todoForm.modalEstTime.options[estTimeIndex].textContent;
...
function renderTodos(todo) {
  ...
  let existingTodoIndex = todoStorage.findIndex(
    (item) => JSON.stringify(item) === JSON.stringify(todo)
  );
...
function filterStatus() {
  ...
  statusFilter.addEventListener("change", () => {
    ...
    todoItems.forEach((todoItem) => {
      let isDoneCheckbox = todoItem.querySelector(
        'input[type="checkbox"]'
      ).checked;
